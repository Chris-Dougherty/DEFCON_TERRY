...
public class TankUnit : MonoBehaviour {
    ...
    void DetectOthers()
    {
        ...
        {
            if (hits[i].gameObject.tag == "EnemyUnitTank" || hits[i].gameObject.tag == "EnemyUnit")
            {
                NextFire = Time.time + FireRate;
                //hits[i].gameObject.GetComponent<EnemyTankUnit>().TakeDamage(1);
                Shoot(hits[i]);
            }
        }
    }
    ...
    void Shoot(Collider c)
    {
        ...
        if (Time.time > fireRate + lastShot)
        {
            ...
            if (c.gameObject.tag == "EnemyUnitTank")
                c.gameObject.GetComponent<EnemyTankUnit>().TakeDamage(10);
