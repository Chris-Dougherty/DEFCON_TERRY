...
public class EnemyPlaneUnit : MonoBehaviour {
    ...
    void DetectOthers()
    {
        ...
        {
            if (hits[i].gameObject.tag == "PlayerUnit" || hits[i].gameObject.tag == "PlayerUnitPlane")
            {
                NextFire = Time.time + FireRate;
                //hits[i].gameObject.GetComponent<PlaneUnit>().TakeDamage(1, Group);
                Shoot(hits[i]);
            }
        }
    }
    ...
    void Shoot(Collider c)
    {
        ...
        if (Time.time > fireRate + lastShot)
        {
            ...
            if (c.gameObject.tag == "PlayerUnit")
                c.gameObject.GetComponent<TransportUnit>().TakeDamage(5, Group);
            else
                c.gameObject.GetComponent<PlaneUnit>().TakeDamage(5, Group);
